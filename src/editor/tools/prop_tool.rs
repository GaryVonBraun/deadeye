    ...
    match action {
        ToolAction::Place(name) => {
            ...
            if editor_settings.snap_to_grid {
                ...
                placement_position = prop_tile_aligned(world_pos, active_map, size);
            } else {
...
pub fn prop_tile_aligned(world_pos: Vec2, active_map: ResMut<ActiveMap>, size: Vec2) -> Vec2 {
    ...
    tile_world_position.x = (tile_world_position.x + size.x / 2.) - active_map.tileset.tile_size;
    tile_world_position.y = (tile_world_position.y + size.y / 2.) - active_map.tileset.tile_size;
